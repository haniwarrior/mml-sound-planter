...
const song = (mml:string,defs='')=>compile(`track0 {${defs}} track1 {@s,0 ${mml}}`)
...
test('case, whitespace and original comment locations',()=>{
 ...
 const s=new Sequencer(data.tracks[1]);const c=s.next()!;assert.equal(c.pitch,61);close(c.duration,240/128/3*1.75);assert.equal(c.pos.line,4);assert.equal(c.pos.column,8);assert.equal(s.next()!.pitch,61)
 assert.throws(()=>compile('track0 {}\n// comment\ntrack1 {@s,0 z}'),(e:unknown)=>e instanceof MmlError && e.position.line===3)
})
...
test('all tracks share origin; tempo changes are track-local',()=>{
 const s=compile('track0 {} track1 {@s,0 t120 c t60 d} track11 {@s,0 t60 c d}')
...
test('test playback uses definitions and only the test track; independent defaults',()=>{
 const main='track0 {@e,1 {31,0,0,15,0,2}} track1 {@s,0 o8 v1 c}'
 const s=compile(main,'@s,0 @e,1 c');assert.deepEqual(Object.keys(s.tracks),['1']);const e=new Sequencer(s.tracks[1]).next()!;assert.equal(e.pitch,60);assert.equal(e.state.volume,96)
...
for(const source of [
 'track1 {@s,0 c}','track0 {} track0 {}','track0 {} track1 {} track1 {}','track0 {} track12 {}',
 'track0 {@e,0 {31,0,0,15,0,0}}','track0 {@e,1 {31,0,0,15,0,0} @e,1 {31,0,0,15,0,0}}',
 'track0 {@e,1 {31,0,0,15,0}}','track0 {@e,1 {31,0,0,15,0,0,1}}','track0 {@e,1 {32,0,0,15,0,0}}',
 'track0 {@e,1 {31,32,0,15,0,0}}','track0 {@e,1 {31,0,32,15,0,0}}','track0 {@e,1 {31,0,0,16,0,0}}',
 'track0 {@e,1 {31,0,0,15,16,0}}','track0 {@e,1 {31,0,0,15,0,4}}','track0 {c}','track0 {} track1 {c}'
...
test('parser produces structured loops rather than expanded source',()=>{
 const s=parseSong('track0 {} track1 {@s,0 [c]0}');assert.equal(s.tracks[1][1].kind,'loop')
})
