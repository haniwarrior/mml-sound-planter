import type { Envelope, Lfos, FmTone } from '../mml/ast.ts'
...
export class TrackVoice implements Voice {
  ...
  constructor(privateSampleRate: number, envelopes: Record<number,Envelope>, lfos:Lfos = {'@lv':{},'@lt':{}}, fmTones:Record<number,FmTone> = {}) {this.fm=new FmGenerator(privateSampleRate);this.fmTones=fmTones;this.lfos=lfos;this.sampleRate=privateSampleRate;this.envelopes=envelopes;this.eg=new EnvelopeGenerator(BYPASS,60)}
  ...
  start(event:TimedEvent) {
    ...
    this.eg.configure(this.definition,(event.state.synth==='fm' || event.state.mode===0 || event.state.mode>=101) ? event.pitch! : 60)
  ...
  sample(time:number) {
    ...
    this.eg.configure(this.definition,(e.state.synth==='fm' || e.state.mode===0 || e.state.mode>=101) ? pitch : 60)
