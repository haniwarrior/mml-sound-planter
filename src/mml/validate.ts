  ...
  const run = (nodes: Node[], input: State, last = false): Result => {
    ...
    for (const n of nodes) {
      ...
      if (n.kind === 'macro') {
        ...
        if (n.kind === 'set') { octave(n); if (n.command === '@s' || n.command === '@f' || n.command === '@w') state.mode = true; if(n.command === '@w' && !song.waveTones[n.value]) throw new MmlError(`undefined wave tone: @w${n.value}`,n.pos); if(n.command === '@f' && !song.fmTones[n.value]) throw new MmlError(`undefined FM tone: @f${n.value}`,n.pos) }
        if (n.kind === 'tie') {
        ...
        if (n.kind === 'note' || n.kind === 'portamento') {
          ...
          if (!isRest && !state.mode) throw new MmlError('発音前に@sN、@fNまたは@wNを指定してください',n.pos)
