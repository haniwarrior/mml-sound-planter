import { MmlError, type Position, type SourceId } from './ast.ts'
...
export function lex(source: string, sourceId: SourceId = 'main'): Token[] {
  const tokens: Token[] = []
  ...
  while (offset < source.length) {
    ...
    const pos: Position = { offset, line, column, source: sourceId }
  ...
  tokens.push({ text: '', pos: { offset, line, column, end: offset, source: sourceId } })
  return tokens
}
