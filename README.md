...
Main/Test共通のbeforeinput処理で、直接入力した`$`と`(`をペアにし、直後の既存`$`/`)`は文字を増やさず越えます。選択中の文字列はペアで囲みます。`@eN`・`@fN`・`@wN`・`@ltN`・`@lvN`直後（空白可）の`{`で定義テンプレートを挿入し、キャレットを最初の0の直後に置きます。音色番号の範囲は既存parserと同じです。コメント・マクロ名内では定義テンプレートを生成しません。

FMは42値、波形メモリは32値。内部行は現在行の行頭インデントに2スペースを加え、閉じ括弧は元のインデントに揃え、最後に改行を1個付けます。後続文字列は保持します。LFOテンプレートのPeriodも指定どおり0なので、再生前に有効値へ編集してください。

Paste・IME composition・Undo/Redoイベントは補完対象外です。トリガー文字と補完を既存のexecCommandによる1回の挿入にまとめます。非対応環境では従来のsetRangeTextへフォールバックするため、Undo履歴の扱いはブラウザ依存です。既存の行番号・エラー表示・ガイド更新は共通inputイベントを利用します。

`track0`〜`track12`直後（空白可）の`{`も、同じ補完処理でブロックへ展開します。内部行は行頭インデント＋2スペース、閉じ括弧は元のインデントで、キャレットは内部行のインデント直後に置きます。閉じ括弧の後に改行は追加せず、後続文字を保持します。Paste/IME除外・Undo方式は定義テンプレートと共通です。

専用テンプレートに一致しない直接入力の`{`は`{}`へ補完し、キャレットを間に置きます。FM/波形テンプレートの直後が空白・タブと改行だけなら、その行末の改行1個をテンプレート末尾の改行とまとめます。次行のインデント・実データ・元からある追加の空行は保持します。この置換もテンプレート挿入と同じ1回の編集操作です。
